...
const registerTasks = async () => {
  if (!(await TaskManager.isAvailableAsync())) {
    console.debug('[geofence] TaskManager is not available on this platform/build');
    return;
  }
  ...
  TaskManager.defineTask(GEOFENCE_TASK, async ({ data, error }) => {
    ...
    if (response?.action === 'notify') {
      await Notifications.scheduleNotificationAsync({
        content: {
          title: 'BlitzPay Merchant',
          body: `You are near your branch: ${regionId}.`,
        },
        trigger: null,
      });
    }
      ...
      if (dist <= region.radius) {
        ...
        if (response?.action === 'notify') {
          await Notifications.scheduleNotificationAsync({
            content: {
              title: 'BlitzPay Merchant',
              body: `You are near your branch: ${region.merchantName}.`,
            },
            trigger: null,
          });
        }
